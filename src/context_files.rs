...
mod tests {
    ...
    fn test_context_loader() {
        ...
        // May be empty in test environment, just verify it doesn't panic
        assert!(files.len() >= 0); // Vec always has len >= 0
    }
...
"#;
    ...
    #[test]
    fn test_parse_claude_md_empty() {
        let sections = parse_claude_md("");
        assert!(sections.is_empty());
    }

    #[test]
    fn test_generate_claude_md_template() {
        let template = generate_claude_md_template("test-project", "cli");
        assert!(template.contains("Project Guidelines"));
    ...
    fn test_context_file_type() {
        assert!(matches!(ContextFileType::Agents, ContextFileType::Agents));
        assert!(matches!(ContextFileType::Claude, ContextFileType::Claude));
    }

    #[test]
    fn test_build_system_prompt() {
        let loader = ContextLoader::new(PathBuf::from("/tmp"));
        let prompt = loader.build_system_prompt();
        // Result should be ok (empty string or with content)
        assert!(prompt.is_ok());
    }

    #[test]
    fn test_context_loader_with_nonexistent_dir() {
        let loader = ContextLoader::new(PathBuf::from("/nonexistent/path"));
        let files = loader.load_all().unwrap_or_default();
