...
impl MemorySystem {
    ...
    pub fn new(home: Option<PathBuf>) -> Result<Self> {
        let db_path = home
            .unwrap_or_else(|| crate::dirs::memory_dir().unwrap_or_else(|| PathBuf::from("~/.code-buddy/memory/memory.db")))
            .join("memory.db");
    ...
    pub fn get_profile(&self, category: Option<&str>) -> Result<Vec<UserProfile>> {
        ...
        let query = if let Some(cat) = category {
            format!("SELECT id, category, key, value, confidence, source, created_at FROM user_profiles WHERE category = '{}'", cat)
        } else {
            "SELECT id, category, key, value, confidence, source, created_at FROM user_profiles".to_string()
        };
