...
pub async fn execute_code(
    ...
    // Build command - get path as string with proper error handling
    let file_path_str = file_path.to_str().ok_or_else(|| {
        anyhow::anyhow!("File path contains invalid UTF-8 characters")
    })?;

    // Track binary path for cleanup (Rust compilation)
    let mut cleanup_binary: Option<PathBuf> = None;

    let (cmd, args) = match language {
        Language::Bash | Language::Shell => ("bash".to_string(), vec![file_path_str.to_string()]),
        Language::Python | Language::Python3 => ("python3".to_string(), vec![file_path_str.to_string()]),
        Language::JavaScript | Language::Node => ("node".to_string(), vec![file_path_str.to_string()]),
        Language::Ruby => ("ruby".to_string(), vec![file_path_str.to_string()]),
        Language::PHP => ("php".to_string(), vec![file_path_str.to_string()]),
        Language::Go => ("go".to_string(), vec!["run".to_string(), file_path_str.to_string()]),
        Language::Rust => {
            // Use unique binary name to prevent race conditions with concurrent executions
            let binary_id = nanoid::nanoid!(8);
            let binary_path = temp_dir.join(format!("sandbox_binary_{}", binary_id));
            cleanup_binary = Some(binary_path.clone());
            ...
            let binary_path_str = binary_path.to_str().ok_or_else(|| {
                anyhow::anyhow!("Binary path contains invalid UTF-8 characters")
            })?;
            ("timeout".to_string(), vec![
                format!("{}", config.timeout_secs),
                binary_path_str.to_string()
            ])
        }
    };
    ...
    // Clean up compiled Rust binary if created
    if let Some(ref binary_path) = cleanup_binary {
...
pub fn execute_code_sync(
    ...
    // Reuse a single Tokio runtime across calls to avoid the cost of creating one each time.
    // Store Result to handle creation errors gracefully (errors stored = runtime not available).
    static RUNTIME: OnceLock<Result<tokio::runtime::Runtime, std::io::Error>> = OnceLock::new();
    let runtime = RUNTIME.get_or_init(tokio::runtime::Runtime::new);
    let runtime = match runtime {
        ...
        Err(e) => {
            return ExecutionResult {
                ...
                error: Some(format!("Failed to create Tokio runtime: {}. This may indicate nested async runtime not supported.", e)),
                truncated: false,
            };
        }
    };

    runtime.block_on(execute_code(code, language, config)).unwrap_or_else(|e| {
        ExecutionResult {
            success: false,
            stdout: String::new(),
            stderr: String::new(),
...
pub fn quick_exec(code: &str, language: &str) -> ExecutionResult {
    let lang = Language::from_language_str(language).unwrap_or(Language::Bash);
...
mod tests {
    ...
    fn test_language_detection() {
        assert_eq!(Language::from_language_str("python"), Some(Language::Python3));
        assert_eq!(Language::from_language_str("js"), Some(Language::JavaScript));
        assert_eq!(Language::from_language_str("shell"), Some(Language::Bash));
        assert_eq!(Language::from_language_str("unknown"), None);
    }
    ...
    fn test_rust_execution() {
        ...
        let config = SandboxConfig::default();
        let result = execute_code_sync(r#"
fn main() {
    println!("Hello from Rust!");
}
"#, &lang, &config);
        // Rust compilation may fail in sandbox, but shouldn't panic
    ...
    fn test_output_truncation() {
        let lang = Language::Bash;
        let mut config = SandboxConfig::default();
        config.max_output_chars = 10;
        let result = execute_code_sync("echo 'This is a very long output'", &lang, &config);
