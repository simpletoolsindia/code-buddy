...
impl HookContext {
    ...
    /// SECURITY: Values substituted into templates are shell-escaped to prevent injection
    pub fn expand(&self, template: &str) -> String {
        ...
        // Expand {path} - shell escape to prevent path traversal/injection
        if let Some(ref path) = self.path {
            result = result.replace("{path}", &shell_escape(path));
        }
        ...
        // Expand {command} - shell escape to prevent injection
        if let Some(ref cmd) = self.command {
            result = result.replace("{command}", &shell_escape(cmd));
        }

        // Expand {error} - shell escape to prevent injection
        if let Some(ref error) = self.error {
            result = result.replace("{error}", &shell_escape(error));
        }

        // Expand {cwd}
        if let Some(ref cwd) = self.cwd {
            result = result.replace("{cwd}", &shell_escape(cwd));
        }
...
/// Shell-escape a string for safe use in sh -c contexts
...
mod tests {
    ...
    fn test_context_expand() {
        ...
        // {path} is shell-escaped for security
    ...
    fn test_shell_escape() {
        ...
        let ctx = HookContext::new("before_write")
            .with_command("echo hello; rm -rf /")
            .with_error("Connection refused: 127.0.0.1:8080");

        // Command and error values are shell-escaped
        assert_eq!(ctx.expand("{command}"), "'echo hello; rm -rf /'");
        assert_eq!(ctx.expand("{error}"), "'Connection refused: 127.0.0.1:8080'");

        // Test that single quotes in values are properly handled
        let ctx2 = HookContext::new("before_write")
            .with_path("It's a path");
        assert_eq!(ctx2.expand("{path}"), "'It'\\''s a path'");
    }
