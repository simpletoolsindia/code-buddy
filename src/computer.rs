...
impl ComputerUse {
    ...
    fn mouse_move(&self, x: i32, y: i32) -> ComputerResult {
        #[cfg(target_os = "macos")]
        {
            // SECURITY: Validate coordinates to prevent injection via osascript
            if x < -32768 || x > 32767 || y < -32768 || y > 32767 {
                return ComputerResult::error("mouse_move", "Coordinates out of valid range");
            }
    ...
    fn type_text(&self, text: &str) -> ComputerResult {
        #[cfg(target_os = "macos")]
        {
            // SECURITY: Validate text to prevent injection via clipboard/osascript
            if text.contains('\0') {
                return ComputerResult::error("type_text", "Null bytes not allowed");
            }
            // Use Python to safely handle all special characters
            let python_script = format!(
                "python3 -c 'import sys; sys.stdout.buffer.write(\"{}\".encode(\"utf-8\"))'",
                text.replace("\\", "\\\\").replace("\"", "\\\"").replace("\n", "\\n").replace("\r", "\\r")
            );
            let output = std::process::Command::new("sh")
                .args(["-c", &format!("{} | pbcopy", python_script)])
                .output();
        ...
        #[cfg(target_os = "windows")]
        {
            // SECURITY: Validate text to prevent injection via PowerShell SendKeys
            if text.contains('\0') || text.contains('{') || text.contains('}') {
                return ComputerResult::error("type_text", "Invalid characters for SendKeys");
            }
            let output = std::process::Command::new("powershell")
    ...
    fn key_combo(&self, keys: &[String]) -> ComputerResult {
        // SECURITY: Validate keys to prevent injection via osascript
        for key in keys {
            if key.contains('"') || key.contains('\'') || key.contains('\\') || key.contains('\n') {
                return ComputerResult::error("key_combo", "Invalid key character");
            }
        }
        let combo = keys.join("+");
        ...
        #[cfg(target_os = "windows")]
        {
            // SECURITY: Validate keys to prevent injection via PowerShell SendKeys
            let joined_keys = keys.join("");
            if joined_keys.contains('\0') || joined_keys.contains('{') || joined_keys.contains('}') {
                return ComputerResult::error("key_combo", "Invalid characters for SendKeys");
            }
            let output = std::process::Command::new("powershell")
                .args(["-Command", &format!("Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait('^{}')", joined_keys)])
                .output();
