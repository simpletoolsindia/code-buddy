...
impl MlxConfig {
    ...
    pub async fn download_model(&self, model_id: &str) -> Result<()> {
        ...
        // SECURITY: Escape model_id for safe use in Python single-quoted string context.
        // Use JSON serialization which handles all special characters (quotes, backslashes, newlines, etc.)
        let model_id_json = serde_json::to_string(model_id)
            .context("Failed to serialize model ID")?;
        // JSON strings are always double-quoted, strip the quotes to use in single-quoted Python context
        let model_id_escaped = model_id_json.trim_matches('"')
            .replace("\\'", "'")  // Handle any escaped single quotes from JSON
            .replace("\\\\", "\\"); // Handle double-escaped backslashes

        let output = Command::new(python)
            .args(["-c", &format!("from mlx_lm.hub import download; download('{model_id_escaped}')")])
            .output()
    ...
    pub async fn generate(&self, prompt: &str, model: &str) -> Result<String> {
        ...
        // Write script to a temp file to avoid shell injection from prompt interpolation
        let temp_script = tempfile::Builder::new()
            .prefix("mlx_gen_")
            .suffix(".py")
            .tempfile()
        ...
        // Build Python script with prompt as a JSON-safe string literal
        let prompt_json = serde_json::to_string(prompt)
            .context("Failed to serialize prompt")?;

        std::fs::write(script_path, format!(
            "import sys\nfrom mlx_lm import generate\nmodel_path = {model_json}\nresponse = generate(model_path, prompt={prompt_json})\nprint(response, end='', flush=True)\n",
            model_json = serde_json::to_string(model).context("Failed to serialize model name")?,
        ...
        F: Fn(&str) + Send + Sync,
    {
        ...
        std::fs::write(script_path, format!(
            "import sys\nfrom mlx_lm import generate\nmodel_path = {model_json}\nfor chunk in generate(model_path, prompt={prompt_json}, stream=True):\n    print(chunk, end='', flush=True)\n",
