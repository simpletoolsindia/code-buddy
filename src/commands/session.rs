...
impl Session {
    ...
    pub fn age_days(&self) -> u64 {
        let now = SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        (now.saturating_sub(self.last_active)) / 86400
    }
}
...
impl SessionManager {
    pub fn new() -> Self {
        ...
        // Use ~/.codebuddy to avoid conflicts with Claude Code
        let storage_dir = if cfg!(target_os = "macos") {
            PathBuf::from(home).join("Library/Application Support/codebuddy/sessions")
        } else {
            PathBuf::from(home).join(".codebuddy/sessions")
        };
